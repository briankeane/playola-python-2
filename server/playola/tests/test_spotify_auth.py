...
def test_spotify_code(test_app, mocker):
    ...
    mocker.patch("playola.api.spotify_auth.get_or_create_user_from_spotify_code", side_effect=future)
    code = "aasdfasdf"
    response = test_app.get(f"/api/v1/spotify/code?code={code}", allow_redirects=False)
    
    future.assert_awaited_once_with(code)
    assert response.status_code == 302
    assert response.headers["location"] == f"{get_settings().client_base_url}/users/the_spotify_user_id"
